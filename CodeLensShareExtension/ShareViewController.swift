...
//
// Hand-off URLs:
//   codelens://share?file=<jpg-name>          → image (lens tab)
//   codelens://share?text=<percent-encoded>   → text snippet (assistant tab)
//   codelens://share?url=<percent-encoded>    → URL (assistant tab)
//
...
final class ShareViewController: UIViewController {
    ...
    private static let appGroupID = "group.com.codelens.shared"
    private static let urlScheme  = "codelens"
    ...
    private func openMainApp(with url: URL) async {
        ...
        // open being refused (CodeLens not installed / scheme unhandled).
        ...
        guard let app else {
            await dismiss(success: false, error: "Couldn't reach the app to open CodeLens.")
            return
        }
        ...
        await dismiss(success: opened,
                      error: opened ? nil : "CodeLens couldn't be opened. Is it installed?")
    }
