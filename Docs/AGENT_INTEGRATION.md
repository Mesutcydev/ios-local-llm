...
`GET /v1/models` and `GET /v1/models/{id}` also expose local runtime extensions:
`context_window`, `max_context_length`, `max_input_tokens`,
`max_output_tokens`, `default_max_tokens`, and `token_count_mode`. These values
come from the configured runtime/KV limit rather than the model's theoretical
maximum. Requests that cannot fit after complete old conversation groups are
removed return HTTP 400 with an OpenAI error code of
`context_length_exceeded`; the current request is never silently chopped.
Successful responses expose token-budget diagnostics through
`X-Token-Usage-Approx`, `X-Token-Count-Mode`, `X-Model-Context-Window`,
`X-Original-Prompt-Tokens`, `X-Effective-Prompt-Tokens`,
`X-Requested-Max-Tokens`, `X-Effective-Max-Tokens`, `X-Input-Truncated`, and
`X-Truncated-Message-Count`. Streaming responses place final completion usage
in the optional usage SSE chunk.

For Aider, use the authenticated `GET /v1/aider/config` route. It returns
`.aider.model.metadata.json` and `.aider.model.settings.yml` content keyed by
the exact `openai/<model-id>` name. The settings YAML contains a real
`extra_params.max_tokens` request parameter; placing that JSON in Aider's chat
would only make it prompt text. See [Aider setup](../AIDER_SETUP.md).
