...
enum LegalDocuments {
    ...
    // MARK: - Public URLs
    static let privacyPolicyURL = "https://github.com/Mesutcydev/ondeviceios/blob/main/PRIVACY_POLICY.md"
    static let eulaURL          = "https://github.com/Mesutcydev/ondeviceios/blob/main/LICENSE"
    static let supportURL       = "https://github.com/Mesutcydev/ondeviceios/issues"
    ...
    **Last updated:** \(currentVersionDate)
    ...
    CodeLens ("we", "the app") is designed with privacy as the core principle. All AI processing happens entirely on your device. We do not operate any servers and we do not collect, store, transmit, or sell your personal data.
    ...
    There is no CodeLens telemetry channel, remote configuration service, analytics SDK, or advertising SDK.
    ...
    **Last updated:** \(currentVersionDate)
    ...
    CodeLens is open-source software. This notice summarizes the licenses and important safety information; it does not add copyright restrictions to the MIT-licensed portions of the app.

    ## 1. CodeLens license
    Original CodeLens source code and documentation are licensed under the **MIT License**. Subject to that license, you may use, copy, modify, merge, publish, distribute, sublicense, and sell copies.

    The MIT copyright and permission notice must be included in copies or substantial portions of the software. The CodeLens name and logos are not licensed as trademarks and must not be used to imply endorsement of a modified distribution.

    ## 2. Third-party components and models
    CodeLens includes or downloads components that are separate works under their own terms. Those terms are not replaced by the CodeLens MIT License.
    ...
    **You alone are responsible for verifying any AI output before relying on it.** Do not use AI output to make any decision that could affect a person's health, safety, finances, legal rights, or employment, without independent verification by a qualified human professional. **Do not run AI-generated code in production, against real data, or in any environment you cannot afford to lose, without thorough independent review and testing.**
    ...
    ## 7. No warranty
    CODELENS, THIRD-PARTY COMPONENTS, MODELS, AND AI OUTPUTS ARE PROVIDED **"AS IS"**, WITHOUT WARRANTY OF ANY KIND, TO THE MAXIMUM EXTENT PERMITTED BY THEIR APPLICABLE LICENSES AND BY LAW. The complete MIT warranty disclaimer is included with the source distribution.
    ...
    **By continuing, you confirm that you have read this notice and understand that model licenses and safety information require separate attention.**
    ...
    CodeLens runs **third-party artificial intelligence models entirely on your device** to analyse images, generate text, extract code, review code, transcribe speech, and synthesize speech. A model's availability in the catalog does not mean it is open source. Read its license and understand the following before relying on any output.
    ...
    **By tapping "I Understand", you confirm that you have read this disclaimer and accept full responsibility for how you use the app and AI output.**
    ...
    On-device AI inference is computationally intensive. By using CodeLens you acknowledge and accept the following.
    ...
    These are intrinsic properties of on-device machine learning, not defects of CodeLens. By continuing you accept that we are not liable for normal thermal output, normal battery drain, or accelerated battery wear consistent with intensive use.
    """
