...
enum WipeAllDataService {
    ...
    static func wipeAll() -> Receipt {
        ...
        let keysToWipe = [
            "codelens.snippets.v1",
            "codelens.memory.v1",
            "codelens.benchmark.history.v1",
            "codelens.metrickit.entries.v1",
            "codelens.recentPrompts",
        ]
        // Snippet / memory counts (best-effort)
        if let snipData = defaults.data(forKey: "codelens.snippets.v1"),
           let snips = try? JSONSerialization.jsonObject(with: snipData) as? [Any] {
            r.snippetsDeleted = snips.count
        }
        if let memData = defaults.data(forKey: "codelens.memory.v1"),
