...
final class BridgeManager: ObservableObject {
    ...
    func pairWithMac(qrJSON: String) async {
        guard let data = qrJSON.data(using: .utf8),
              ...
              payload.v == 1 else {
            pairingPhase = .failed("Unrecognised QR — scan a CodeLens Mac QR.")
            return
        }
        ...
        if payload.macCertFingerprint == nil && !AppSettings.shared.allowInsecureBridgePairing {
            pairingPhase = .failed("This Mac doesn't support encrypted pairing. Update the CodeLens Mac app, or enable “Allow insecure pairing” in Settings → Mac to continue over your local network.")
            return
        }
