...
final class LocalAPIServerTests: XCTestCase {
    ...
    func testAdvertisedInterfacesIncludeTailscaleAndExcludeCellular() {
        ...
        XCTAssertTrue(LocalAPIValidation.isReachableLANInterface("utun4"))
        ...
        XCTAssertFalse(LocalAPIValidation.isReachableLANInterface("lo0"))
        XCTAssertFalse(LocalAPIValidation.isReachableLANInterface("awdl0"))
        XCTAssertFalse(LocalAPIValidation.isReachableLANInterface("llw0"))
    }
    ...
    func testTokenBudgetReservesPromptOutputAndSafetyTokens() throws {
        let prompt = LocalAPITokenCount(tokens: 5_000, mode: .exact)
        let decision = try LocalAPITokenBudget.makeDecision(
            contextWindow: 8_192,
            ...
            maxTokensSource: .maxCompletionTokens,
            inputWasTruncated: false,
            truncatedMessageCount: 0,
            truncatedGroupCount: 0
        )
        XCTAssertEqual(decision.effectiveMaxTokens, 2_048)
        XCTAssertLessThanOrEqual(decision.totalReservedTokens, 8_192)
        XCTAssertEqual(decision.headers["X-Input-Truncated"], "false")
    }

    func testTokenBudgetRejectsInvalidAndOverMaximumOutput() {
        XCTAssertThrowsError(try LocalAPITokenBudget.resolveOutputTokens(
            requested: 0,
            source: .maxTokens,
            maximum: 2_048,
            defaultValue: 1_024,
        ...
        XCTAssertEqual(
            try LocalAPITokenBudget.resolveOutputTokens(
                requested: 4_096,
    ...
    func testHistorySelectorRemovesOldestCompleteGroupsAndPreservesNewestMessage() async throws {
        let oldUser = ChatMessage(role: .user, content: String(repeating: "old ", count: 100))
        let oldAssistant = ChatMessage(role: .assistant, content: String(repeating: "answer ", count: 100))
        let current = ChatMessage(
            role: .user,
            content: "Current visible request",
            modelContent: String(repeating: "current ", count: 40)
        )
        let messages = [
        ...
        let counter: LocalAPIHistorySelector.TokenCounter = { values in
            LocalAPITokenCount(
                tokens: values.reduce(0) { $0 + max(1, $1.contentForModel.count / 4) },
                mode: .exact
            )
        }
        let selection = try await LocalAPIHistorySelector.select(
            messages: messages,
            promptBudget: 100,
            counter: counter
        )
        XCTAssertTrue(selection.didTruncate)
        XCTAssertEqual(selection.messages.first?.role, .system)
        XCTAssertEqual(selection.messages.last?.content, current.content)
        XCTAssertEqual(selection.messages.last?.modelContent, current.modelContent)
        XCTAssertFalse(selection.messages.contains { $0.id == oldUser.id })
        XCTAssertFalse(selection.messages.contains { $0.id == oldAssistant.id })
    }

    func testHistorySelectorKeepsToolCallAndResultAtomic() async throws {
        let call = ChatMessage.ToolCallMetadata(
            id: "call_1",
            name: "terminal",
            argumentsJSON: #"{"command":"pwd"}"#
        )
        ...
        let current = ChatMessage(role: .user, content: "Continue")
        let messages = [
        ...
        let counter: LocalAPIHistorySelector.TokenCounter = { values in
            LocalAPITokenCount(tokens: values.count * 10, mode: .exact)
        }
        let selection = try await LocalAPIHistorySelector.select(
            messages: messages,
            promptBudget: 40,
        ...
        let selectedIDs = Set(selection.messages.map(\.id))
        XCTAssertFalse(selectedIDs.contains(assistant.id))
        XCTAssertFalse(selectedIDs.contains(result.id))
        XCTAssertTrue(selectedIDs.contains(current.id))
    }

    func testAiderExportUsesExactQualifiedModelAndRealOutputSetting() throws {
        let profile = ModelCapabilityProfile.resolve(
            repoID: "local_Qwen3-7",
            catalogContextLength: 8_192,
            supportsThinking: true
        )
        let export = AiderConfigurationExporter.export(
            modelID: "imported:local/local_Qwen3-7",
            profile: profile,
            supportsFunctionCalling: false,
            supportsVision: false
        )
        XCTAssertEqual(export.modelName, "openai/imported:local/local_Qwen3-7")
        XCTAssertFalse(export.metadataJSON.contains("openai/openai/"))
        XCTAssertTrue(export.settingsYAML.contains("extra_params:"))
        XCTAssertTrue(export.settingsYAML.contains("max_tokens: \(profile.maximumOutputTokens)"))
        let metadata = try XCTUnwrap(export.metadataObject[export.modelName])
        XCTAssertEqual(metadata.maxInputTokens, profile.effectiveContextWindow)
        XCTAssertEqual(metadata.inputCostPerToken, 0)
        XCTAssertEqual(metadata.litellmProvider, "openai")
    }

    func testChunkFrameWrapsBodyInHTTPEncoding() {
        let body = Data("data: {\"delta\":\"hi\"}\n\n".utf8)
        let framed = LocalAPIServer.chunkFrame(body)
        let expected = Data(String(body.count, radix: 16).utf8)
            + Data("\r\n".utf8) + body + Data("\r\n".utf8)
        XCTAssertEqual(framed, expected)
        XCTAssertTrue(LocalAPIServer.chunkFrame(Data()).isEmpty)
    }

    func testChunkedRequestBodyIsFlaggedForLengthRequired() throws {
        let chunked = HTTPRequest(data: Data(
            ("POST /v1/chat/completions HTTP/1.1\r\n"
                + "Host: phone.local\r\n"
                + "Transfer-Encoding: chunked\r\n"
                + "\r\n").utf8
        ))
        XCTAssertTrue(try XCTUnwrap(chunked).declaresChunkedBody)

        let contentLength = HTTPRequest(data: Data(
            ("POST /v1/chat/completions HTTP/1.1\r\n"
                + "Host: phone.local\r\n"
                + "Content-Length: 2\r\n"
                + "\r\n{}").utf8
        ))
        XCTAssertFalse(try XCTUnwrap(contentLength).declaresChunkedBody)

        let identity = HTTPRequest(data: Data(
            ("POST /v1/chat/completions HTTP/1.1\r\n"
                + "Host: phone.local\r\n"
        ...
        XCTAssertFalse(try XCTUnwrap(identity).declaresChunkedBody)
    }

    func testAnthropicAndResponsesPayloadsCarryRealUsage() throws {
        let result = AssistantGenerationResult(
            promptTokenCount: 21,
            completionTokenCount: 7,
            stopReason: .stop
        )
        let usage = LocalAPIUsage(result)

        let anthropic = try JSONSerialization.jsonObject(
            with: LocalAPIResponse.anthropicMessage(
                id: "msg_1",
                model: "test-model",
                text: "hi",
                usage: usage
        ...
        let anthropicUsage = try XCTUnwrap(anthropic?["usage"] as? [String: Int])
        XCTAssertEqual(anthropicUsage["input_tokens"], 21)
        XCTAssertEqual(anthropicUsage["output_tokens"], 7)
        ...
        let responsesUsage = try XCTUnwrap(responses?["usage"] as? [String: Int])
        XCTAssertEqual(responsesUsage["input_tokens"], 21)
        XCTAssertEqual(responsesUsage["output_tokens"], 7)
        XCTAssertEqual(responsesUsage["total_tokens"], 28)
    }

    func testHermesModelFamilyGetsHermesToolParser() {
        let profile = ModelCapabilityProfile.resolve(
            repoID: "NousResearch/Hermes-4-14B",
            catalogContextLength: 131_072,
            supportsThinking: true
        )
        XCTAssertEqual(profile.family, .hermes)
        XCTAssertEqual(profile.toolParser, .hermes)
        XCTAssertEqual(profile.reasoningParser, .hermes)
        XCTAssertFalse(profile.reasoningParser.assumesPrefilledOpening)
        XCTAssertTrue(profile.isHermesContextCompatible)

        // Small-context Hermes builds keep the parser but fail the
        // Hermes-agent context floor, matching the other families.
        let smallProfile = ModelCapabilityProfile.resolve(
            repoID: "local/hermes-3-8b",
            catalogContextLength: 8_192,
            supportsThinking: true
        )
        XCTAssertEqual(smallProfile.family, .hermes)
        XCTAssertFalse(smallProfile.isHermesContextCompatible)
        XCTAssertNotNil(smallProfile.hermesWarning)
    }

    func testToolDecisionBuffersThroughReasoningPrefix() {
        // Mirrors the streaming tool path: the buffering decision runs on the
        // post-reasoning view, so a thinking model's <think> block does not
        // flip the stream to prose before the tool envelope appears.
        var filter = LocalAPIReasoningFilter()
        var visible = ""
        for token in [
            ...
            visible += filter.consume(token).content
        }
        XCTAssertTrue(
            LocalAPIToolCalling.shouldBufferForToolDecision(visible),
            "reasoning followed by a tool envelope must stay buffered"
        )

        var proseFilter = LocalAPIReasoningFilter()
        var prose = ""
        for token in [
            "<think>", "Reasoning briefly ", "about the answer. ", "</think>",
            "The weather today is sunny with a light breeze all afternoon."
        ] {
            prose += proseFilter.consume(token).content
        }
        XCTAssertFalse(
            LocalAPIToolCalling.shouldBufferForToolDecision(prose),
            "reasoning followed by prose must stream as text"
        )
    }

    func testToolDecisionBuffersJSONArrayForm() {
        XCTAssertTrue(
            LocalAPIToolCalling.shouldBufferForToolDecision(
                "[{\"name\":\"terminal\",\"arguments\":"
            )
        )
        // A bracket-opening prose answer still escapes the buffer once the
        // prefix proves it carries no tool envelope.
        XCTAssertFalse(
            LocalAPIToolCalling.shouldBufferForToolDecision(
                "[1] This is just a citation list with ordinary words that keeps going past the window."
            )
        )
    }

    func testFragmentedSplitsArgumentsIntoBoundedDeltas() {
        let arguments = String(repeating: "x", count: 100)
        let fragments = LocalAPIServer.fragmented(arguments)
        XCTAssertEqual(fragments.count, 3)
        XCTAssertEqual(fragments.joined(), arguments)
        XCTAssertEqual(fragments.map(\.count), [48, 48, 4])
        XCTAssertEqual(LocalAPIServer.fragmented("short"), ["short"])
        XCTAssertTrue(LocalAPIServer.fragmented("").isEmpty)
    }

    func testHermesReposUseChatMLTemplate() {
        XCTAssertEqual(
            ChatTemplate.detect(for: "NousResearch/Hermes-4-14B"),
            ChatTemplate.chatML
        )
        XCTAssertEqual(
            ChatTemplate.detect(for: "local/hermes-3-8b"),
            ChatTemplate.chatML
        )
    }

    func testImportedGGUFPromptUsesChatTemplateNotRoleColon() {
        ...
        XCTAssertFalse(prompt.hasPrefix("user:"))

        let gemma = ChatTemplate.gemma.format(messages: messages)
        XCTAssertTrue(gemma.contains("<start_of_turn>user"))
        XCTAssertTrue(gemma.contains("<start_of_turn>model"))
        XCTAssertFalse(gemma.hasPrefix("user:"))
    }

    func testCoreAIPlannerUsesLastUserAndKeepsHistory() throws {
        let messages = [
        ...
        let plan = try XCTUnwrap(CoreAIConversationPlanner.plan(messages: messages))
        XCTAssertEqual(plan.system, "You are helpful.")
        XCTAssertEqual(plan.latestUser, "What's 2+2?")
        XCTAssertEqual(plan.history, [.user("Hi"), .assistant("Hello!")])
    }

    func testCoreAIPlannerRejectsMissingUserTurn() {
