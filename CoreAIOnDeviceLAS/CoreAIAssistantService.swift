...
final class CodingAssistantService: ObservableObject {
    ...
    /// Signature matches the OnDeviceLAS façade so the shared HTTP layer
    /// compiles unchanged across both targets. Core AI emits explicit
    /// reasoning markup (or plain text); it never pre-fills an opening
    /// reasoning tag, so this only reports whether thinking content is
    /// expected for the active model.
    func resolvedThinkingEnabled(forceNoThinking: Bool) -> Bool {
        activeModel.supportsThinking && !forceNoThinking
    }
