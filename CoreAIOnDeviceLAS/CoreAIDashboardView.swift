...
struct CoreAIDashboardView: View {
    ...
    var body: some View {
        TabView(selection: $tab) {
            ...
            NavigationStack {
                LASTerminalView(showsBackButton: false)
            }
    ...
    private var networkBanner: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 8) {
                ...
                .buttonStyle(LASSecondaryButtonStyle())
                // Without a LAN address this would copy a `<device-ip>`
                // placeholder that no client can use.
                .disabled(manager.addresses.isEmpty)
    ...
    private var modelQuickCard: some View {
        VStack(alignment: .leading, spacing: LASDesignTokens.row) {
            // The card body jumps to Models; the action row below stays
            // outside the tappable area so VoiceOver does not read nested
            // buttons and taps on the actions cannot accidentally navigate.
    ...
    /// Before the first install the service's placeholder model name would
    /// read as if a model existed; the card must say what is actually true.
    private var quickCardModelTitle: String {
        if let manifest = store.manifest {
            return manifest.displayName
        }
        return assistant.state == .ready || assistant.state == .generating
            ? assistant.activeDisplayName
            : "No model installed"
    }
    ...
    private var serverCard: some View {
        VStack(alignment: .leading, spacing: LASDesignTokens.row) {
            ...
            if settings.localAPIEnabled, assistant.state != .ready, assistant.state != .generating {
                HStack(alignment: .top, spacing: 8) {
                    Image(systemName: "exclamationmark.triangle.fill")
                        .foregroundStyle(.orange)
                        .font(.caption)
                    Text("The server is up, but requests return 503 until a model is loaded. Load one on the Models tab.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
    ...
    private var apiKeyRow: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 8) {
                ...
                .privacySensitive()
                Spacer(minLength: 4)
                Button(keyVisible ? "Hide API key" : "Reveal API key") {
                    keyVisible.toggle()
                }
                .buttonStyle(LASSecondaryButtonStyle())
                Button("Copy") {
    ...
    private func applyPort() {
        guard let port = UInt16(portText), port >= 1_024 else {
            ...
            ToastCenter.shared.error(
                "Invalid port",
                detail: "The port must be a number between 1024 and 65535."
            )
            return
        }
