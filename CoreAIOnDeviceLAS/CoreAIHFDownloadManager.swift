...
final class CoreAIHFDownloadManager: ObservableObject, Identifiable {
    ...
    private func listRemoteFiles() async throws -> [RemoteFile] {
        ...
        repeat {
            ...
            cursor = Self.nextCursor(fromLinkHeader: http.value(forHTTPHeaderField: "Link"))
    ...
    /// Extracts the `cursor` query parameter from the `rel="next"` link.
    private static func nextCursor(fromLinkHeader header: String?) -> String? {
