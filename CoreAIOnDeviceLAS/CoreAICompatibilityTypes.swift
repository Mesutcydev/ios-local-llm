...
enum ModelToolParser: String, Codable, CaseIterable, Sendable {
    ...
    // Same raw value as the OnDeviceLAS declaration so `/v1/models` reports
    // one stable parser name across both products.
...
enum ModelReasoningParser: String, Codable, CaseIterable, Sendable {
    ...
    var assumesPrefilledOpening: Bool {
        // Core AI emits explicit reasoning markup or plain text — it never
        // pre-fills an opening reasoning tag the way Qwen3 templates do.
        // Assuming a prefilled opening classified every token of a plain
        // answer as reasoning, leaving streaming clients with no visible
        // output at all.
        switch self {
        case .none, .foundationModels: return false
        }
    }
}
...
struct ModelCapabilityProfile: Equatable, Sendable {
    ...
    var effectiveContextWindow: Int {
        min(configuredContextLength, maximumKVCacheTokens)
    }

    var defaultOutputTokens: Int {
        min(maximumOutputTokens, max(1, min(2_048, effectiveContextWindow / 4)))
    }

    var runtimeLimits: LocalAPIRuntimeLimits {
        LocalAPIRuntimeLimits(
            contextWindow: effectiveContextWindow,
            maximumOutputTokens: maximumOutputTokens,
            defaultOutputTokens: defaultOutputTokens,
            limitSource: .manifest,
            tokenizerIdentifier: nil,
            tokenCountMode: .conservativeApproximation
        )
    }
...
final class AppSettings: ObservableObject {
    ...
    // Default to the device's appearance instead of forcing light on first
    // run; LASThemeHost resolves "system" through LASAppearance.
    @AppStorage("coreAI.appearance") var appearance: String = "system"
    @AppStorage("coreAI.themeAccent") var themeAccent: String = KoduTheme.appAccent.rawValue

    var resolvedColorScheme: ColorScheme? {
        (LASAppearance(rawValue: appearance) ?? .system).colorScheme
    }
}
