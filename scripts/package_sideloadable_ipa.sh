...
unzip -l "$output_abs" | grep -q "Payload/$app_name/embedded.mobileprovision" || die "IPA is missing the embedded provisioning profile"
unzip -l "$output_abs" | grep -q "Payload/$app_name/_CodeSignature/CodeResources" || die "IPA is missing CodeResources"
