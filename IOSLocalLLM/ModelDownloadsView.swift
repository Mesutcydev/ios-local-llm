...
struct LASModelDownloadsView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showDocumentsImporter) {
            LocalModelDocumentsImportSheet(appDocumentsName: "On Device: LAS") { url in
                showDocumentsImporter = false
                Task { await importModel(at: url) }
            }
        }
        .alert(
            "Import failed",
    ...
    private var headerCard: some View {
        LASModelLibraryCard(
            ...
            appDocumentsPreferred: appDocumentsPreferred,
            onImportFolder: { beginImport(.folder) },
            onImportFile: { beginImport(.file) },
            onImportAppDocuments: { showDocumentsImporter = true },
            onManageToken: { showingHFToken = true }
        )
    }

    /// ForgeSign-class resigns that leave a wildcard or missing
    /// application-identifier cannot grant open-in-place folder access —
    /// prefer the in-sandbox App Documents flow for folder imports there.
    private var appDocumentsPreferred: Bool {
        !LocalModelDocumentPickerSession.openInPlacePickingIsUsable
    }
    ...
    private func beginImport(_ kind: LocalModelImportService.ImportKind) {
        importError = nil
        importStatus = nil
        // Folder imports use open-in-place picking; when the signing identity
        // cannot receive file-access grants (ForgeSign-class wildcard
        // resigns), route to the in-sandbox App Documents flow instead.
        if kind == .folder && !LocalModelDocumentPickerSession.openInPlacePickingIsUsable {
...
private struct LASModelLibraryCard: View {
    ...
    let appDocumentsPreferred: Bool
    let onImportFolder: () -> Void
    let onImportFile: () -> Void
    let onImportAppDocuments: () -> Void
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: LASDesignTokens.component) {
            ...
            Text(appDocumentsPreferred
                 ? "Sideload build: Files folder-picking is unavailable here — use App Documents."
                 : "Files → On My iPhone → On Device: LAS")
                .font(.caption.monospaced())
                .foregroundStyle(.secondary)
    ...
    private var importMenu: some View {
        Menu {
            if appDocumentsPreferred {
                Button("Import from App Documents", systemImage: "internaldrive", action: onImportAppDocuments)
            } else {
