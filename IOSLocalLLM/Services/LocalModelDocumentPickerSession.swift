...
final class LocalModelDocumentPickerSession: NSObject, UIDocumentPickerDelegate {
    ...
    /// True when open-in-place folder/package picking is expected to work on
    /// this install. Open-in-place hands the app a security-scoped URL to
    /// content *outside* its sandbox, which iOS only reliably grants to App
    /// Store / TestFlight signatures.
    ///
    /// A concrete `application-identifier` is necessary but NOT sufficient:
    /// resigned/sideloaded builds (AltStore, ForgeSign, dev, ad-hoc,
    /// enterprise) can still have the File Provider deny the grant even with a
    /// concrete identity — the folder looks selectable but "Open" does nothing.
    /// Those builds route folder/package imports through the in-sandbox App
    /// Documents browser (`LocalModelDocumentsImportSheet`); single-file
