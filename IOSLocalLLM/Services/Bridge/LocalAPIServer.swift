...
enum LocalAPIValidation {
    ...
    static func isReachableLANInterface(_ name: String) -> Bool {
        // Keep loopback, cellular, AirDrop, and IPsec off the advertised URL
        // list. `utun` is included so Tailscale and WireGuard addresses show
        // up — the local API is bearer-authenticated and intended for a
        // trusted overlay as well as Wi-Fi.
        let excludedPrefixes = ["lo", "ipsec", "awdl", "llw", "pdp_ip"]
...
struct LocalAPIChatRequest {
    ...
    static func decodeOpenAI(_ data: Data) throws -> Self {
        ...
        return Self(
            ...
            maxTokensSource: raw["max_completion_tokens"] != nil
                ? .maxCompletionTokens
                : raw["max_tokens"] != nil ? .maxTokens : .defaultValue,
    ...
    static func decodeOpenAIResponses(_ data: Data) throws -> Self {
        ...
        return Self(
            ...
            maxTokensSource: .maxOutputTokens,
            reasoningPreference: decodeReasoningPreference(raw),
            temperature: number(raw["temperature"]),
            topP: number(raw["top_p"]),
            tools: tools,
...
enum LocalAPIToolCalling {
    ...
    static func shouldBufferForToolDecision(_ output: String) -> Bool {
        ...
        // `[` covers models that emit the tool calls as a JSON array of
        // objects; the balanced-brace scanner resolves the inner calls.
    ...
    static func messages(
        ...
        case .foundationModels:
            // Core AI runs the text protocol: the runtime's native session
            // tools are not wired to the HTTP layer, so instruct the same
            // Hermes envelope the server parser understands.
            responseInstruction = "For a call, output <tool_call>{\"name\":\"function_name\",\"arguments\":{\"argument\":\"value\"}}</tool_call>. For an ordinary answer, respond naturally without a tool-call wrapper."
        case .none:
...
struct LocalAPIUsage: Equatable, Sendable {
    ...
    /// Usage in the field names the Responses API reports.
    var responseObject: [String: Int] {
        [
...
actor LocalAPIServer {
    ...
    // Streaming bodies use HTTP/1.1 chunked framing. Strict clients, SDKs,
    // and LAN proxies treat a close-delimited body as incomplete until the
    // connection drops; an explicit terminating chunk gives every consumer a
    // real end-of-stream marker.
    private var chunkedStreamConnections: Set<ObjectIdentifier> = []
    private var streamingHeadersCommitted: Set<ObjectIdentifier> = []
    ...
    private func handle(_ connection: NWConnection) async {
        ...
        defer {
            ...
            streamingHeadersCommitted.remove(connectionID)
    ...
    private func readRequest(
        ...
        // The timeout bounds connection silence, not the whole upload: a
        // 20 MB image body over slow Wi-Fi can legitimately outlive a fixed
        // end-to-end window. A connection that goes quiet for the full
        // interval is the one that is actually stuck.
        var idleDeadline = Date().addingTimeInterval(requestTimeout)
        while true {
            let remaining = idleDeadline.timeIntervalSinceNow
            ...
            idleDeadline = Date().addingTimeInterval(requestTimeout)
        }
    }
    ...
    private func route(
        ...
        // The request parser only understands Content-Length-delimited
        // bodies. Answering chunked uploads with 411 instead of a misleading
        // "JSON body required" 400 tells SDKs and proxies exactly what to fix.
        if request.method == "POST", request.declaresChunkedBody {
            await error(
                connection,
                status: 411,
                message: "Transfer-Encoding request bodies are not supported. Send the JSON body with a Content-Length header instead.",
        ...
        case ("GET", "/v1/aider/config"):
            await aiderConfiguration(connection)
    ...
    private func run(
        ...
        let snapshot = await MainActor.run { () -> (String, String, Bool, Bool, Bool, Bool, Bool, ModelCapabilityProfile, Bool, Bool, Int) in
            ...
            return (
                ...
                // Only the MLX runtime accepts the native chat-template tool
                // payload. Core AI has no native-template wiring here — its
                // tool requests must use the text protocol so the schemas
                // actually reach the model.
                service.activeModel.runtime == .mlx,
        ...
        let conservativeCounter = LocalAPIConservativeTokenCounter()
        let countMessages: LocalAPIHistorySelector.TokenCounter = { messages in
            ...
            return conservativeCounter.count(messages: prepared)
        }
        let contextWindow = snapshot.7.effectiveContextWindow
        let defaultOutputTokens = min(snapshot.7.defaultOutputTokens, snapshot.10)
        let initialOutput: Int
        do {
            initialOutput = try LocalAPITokenBudget.resolveOutputTokens(
                requested: decoded.maxTokens,
                source: decoded.maxTokensSource,
                maximum: snapshot.10,
                defaultValue: defaultOutputTokens,
                contextWindow: contextWindow,
                promptTokens: 0
            )
        } catch let budgetError as LocalAPITokenBudgetError {
            await respondBudgetError(
                connection,
                error: budgetError,
                dialect: dialect,
                contextWindow: contextWindow,
            ...
            await self.error(connection, status: 400, message: "Invalid output token limit", dialect: dialect)
            return
        }
        let promptBudget = contextWindow - initialOutput - LocalAPITokenBudget.defaultSafetyReserve
        guard promptBudget > 0 else {
            await respondBudgetError(
                connection,
                error: .noOutputCapacity(
                    contextWindow: contextWindow,
                    promptTokens: 0,
                    parameter: decoded.maxTokensSource.rawValue
                ),
                dialect: dialect,
                contextWindow: contextWindow,
                originalPrompt: nil,
        ...
        let history: LocalAPIHistorySelection
        do {
            history = try await LocalAPIHistorySelector.select(
                messages: decoded.messages,
                promptBudget: promptBudget,
                counter: countMessages
            )
        } catch let selectionError as LocalAPIHistorySelectionError {
            switch selectionError {
            case .requiredContentTooLarge(let count):
                await respondBudgetError(
                    connection,
                    error: .contextLengthExceeded(
                        contextWindow: contextWindow,
                        promptTokens: count.tokens,
                        requestedCompletionTokens: initialOutput,
                        parameter: "messages"
                    ),
                    dialect: dialect,
                    contextWindow: contextWindow,
                    originalPrompt: count,
                    effectivePrompt: count
                )
            }
            return
        } catch {
            await self.error(connection, status: 400, message: "Unable to prepare request context", dialect: dialect)
            return
        }
        let effectiveCount = history.effectivePrompt
        let budgetDecision: LocalAPITokenBudgetDecision
        do {
            budgetDecision = try LocalAPITokenBudget.makeDecision(
                contextWindow: contextWindow,
                maximumOutputTokens: snapshot.10,
                defaultOutputTokens: defaultOutputTokens,
                originalPrompt: history.originalPrompt,
                effectivePrompt: effectiveCount,
                requestedMaxTokens: decoded.maxTokens,
                maxTokensSource: decoded.maxTokensSource,
                inputWasTruncated: history.didTruncate,
                truncatedMessageCount: history.removedMessageCount,
                truncatedGroupCount: history.removedGroupCount
            )
        } catch let budgetError as LocalAPITokenBudgetError {
            await respondBudgetError(
                connection,
            ...
            await self.error(connection, status: 400, message: "Unable to resolve request token budget", dialect: dialect)
            return
        }
        ...
        let maxTokens = budgetDecision.effectiveMaxTokens
        ...
        if decoded.stream {
            ...
            if toolCallingEnabled {
                ...
                // Actor-isolated: concurrent heartbeat / delta / finisher
                // callers serialize here and cannot double-write headers.
                func ensureToolHeaders() async {
                    await ensureStreamingHeaders(
                        connection,
                        dialect: dialect,
                        model: snapshot.0,
                        headers: budgetDecision.headers,
                        responseID: requestID,
                        inputTokens: budgetDecision.effectivePrompt.tokens
                    )
                }

                // A tool request emits nothing while the tool-decision buffer
                // holds the stream, and prefill can take many seconds on top.
                // Idle-timeout-prone consumers (browser EventSource, proxies)
                // abort a silent connection. After a grace window long enough
                // for a clean tool decision to keep its real HTTP error
                // status, commit to streaming and keep the connection warm
                // with SSE comments every client ignores.
                let headerModel = snapshot.0
                let headerFields = budgetDecision.headers
                let headerInputTokens = budgetDecision.effectivePrompt.tokens
                let toolHeartbeat = dialect.usesServerSentEvents
                    ? Task { [weak self] in
                        try? await Task.sleep(for: .seconds(3))
                        while !Task.isCancelled {
                            await self?.ensureStreamingHeaders(
                                connection,
                                dialect: dialect,
                                model: headerModel,
                                headers: headerFields,
                                responseID: requestID,
                                inputTokens: headerInputTokens
                            )
                ...
                func emitToolTextDelta(_ delta: LocalAPIReasoningDelta) async throws {
                    ...
                    case .openAIChat:
                        ...
                        payload = Data("data: ".utf8) + LocalAPIResponse.openAIChunk(
                            ...
                            text: delta.content,
                            role: role,
                            reasoningContent: delta.reasoning.isEmpty ? nil : delta.reasoning
                        ) + Data("\n\n".utf8)
                    case .openAIResponses:
                        guard !delta.content.isEmpty else { return }
                        // The message item and its text part are announced
                        // with the first visible delta — a tool-only
                        // response never creates them.
                        let partStart = !toolStartedVisibleContent
                        ...
                        payload = partStart + LocalAPIResponse.openAIResponseEvent(
                ...
                func finishToolTextStream() async {
                    // The trailing filter delta can be empty, but the error
                    // and terminator paths below emit in-band SSE events, so
                    // the streaming headers must exist either way.
                    await ensureToolHeaders()
                    do {
                    ...
                    case .anthropic:
                        let streamUsage = inferenceSignal.result.map(LocalAPIUsage.init)
                        ...
                        try? await send(connection, LocalAPIResponse.anthropicEvent(
                            ...
                            object: [
                                ...
                                "usage": [
                                    "input_tokens": streamUsage?.promptTokens
                                        ?? budgetDecision.effectivePrompt.tokens,
                                    "output_tokens": streamUsage?.completionTokens ?? 0
                                ]
                            ]
                        ))
                ...
                var decisionFilter = LocalAPIReasoningFilter(
                    prefilledOpening: prefilledReasoningOpen
                )
                var decisionContent = ""
                do {
                    for await token in stream {
                        ...
                        output += token
                        // Decide on the post-reasoning view of the stream.
                        // A thinking model can open with a long <think> block
                        // and still end in a tool call — the raw prefix says
                        // "prose", but the tool envelope behind the reasoning
                        // must stay buffered, not stream as visible text.
                        let visibleDelta = decisionFilter.consume(token)
                        decisionContent += visibleDelta.content
                        if !LocalAPIToolCalling.shouldBufferForToolDecision(decisionContent) {
                            streamsAsText = true
                    ...
                    if calls.isEmpty {
                        ...
                        if requiredTool || invalidEnvelope || timedOut || parsedToolCalls.failure != nil {
                            // A fast tool decision stays silent, so the
                            // failure can still carry a real HTTP status —
                            // the same 502 the non-streaming path returns.
                            // Once the heartbeat has committed to a 200, the
                            // error rides the stream instead.
                            let failureMessage = timedOut
                                ? LocalAPIInferencePolicy.toolTimeoutMessage
                                : LocalAPIToolCalling.toolCallErrorMessage(
                                    for: parsedToolCalls,
                                    tools: decoded.tools
                                )
                            if hasCommittedStreamingHeaders(connection) {
                                try? await sendStreamingErrorEvent(
                                    connection,
                                    dialect: dialect,
                                    type: timedOut ? "generation_timeout" : "tool_call_error",
                        ...
                        case .openAIChat:
                            for (index, call) in calls.enumerated() {
                                ...
                                for fragment in Self.fragmented(call.argumentsJSON) {
                                    try await send(
                                        connection,
                                        Data("data: ".utf8)
                                            + LocalAPIResponse.openAIToolCallFragmentChunk(
                        ...
                        case .ollamaGenerate:
                            // Tool calling is not enabled for this dialect
                            // today; keep the NDJSON stream terminated if it
                            // ever is.
                            try? await sendFinal(
                                connection,
                                LocalAPIResponse.ollamaGenerate(
            ...
            func emitOpenAICompatibleDelta(
                ...
                case .openAIResponses:
                    ...
                    guard !delta.content.isEmpty else { return }
                    // Announce the message item and text part with the
                    // first visible delta, mirroring the tool path.
                    let partStart = !startedVisibleContent
                        ? LocalAPIResponse.openAIResponseEvent(
                            "response.output_item.added",
                            fields: [
                                "response_id": requestID,
            ...
            case .openAIResponses:
                ...
                try? await send(connection, LocalAPIResponse.openAIResponseEvent("response.content_part.done", fields: [
                    "response_id": requestID,
                    "item_id": "msg_\(requestID)",
                    "output_index": 0,
                    "content_index": 0,
                ...
                try? await send(connection, LocalAPIResponse.openAIResponseEvent("response.output_item.done", fields: [
                    "response_id": requestID,
                    "output_index": 0,
                    "item": [
                        "id": "msg_\(requestID)",
            ...
            let payload: Data
            let generationUsage = inferenceSignal.result.map(LocalAPIUsage.init)
            switch dialect {
            case .openAIChat:
                let generationResult = inferenceSignal.result
    ...
    private func respondBudgetError(
        _ connection: NWConnection,
        error budgetError: LocalAPITokenBudgetError,
        dialect: Dialect,
        contextWindow: Int,
        ...
        let code: String
        switch budgetError {
        case .invalidRequestedOutput(let parameterName, let value):
            message = "The requested output token limit \(value) is invalid. It must be greater than zero."
            parameter = parameterName
            code = "invalid_value"
        case .outputLimitExceeded(let parameterName, let requested, let maximum):
            message = "The requested output token limit \(requested) exceeds this runtime's maximum of \(maximum) tokens."
            parameter = parameterName
            code = "max_tokens_exceeded"
        case .contextLengthExceeded(_, let promptTokens, let completionTokens, let parameterName):
            message = "This model's maximum context length is \(contextWindow) tokens. The request requires approximately \(promptTokens + completionTokens) tokens: \(promptTokens) prompt tokens plus \(completionTokens) requested completion tokens. Reduce the message content or requested completion length."
            parameter = parameterName
            code = "context_length_exceeded"
        case .noOutputCapacity(_, let promptTokens, let parameterName):
            message = "This model has no output capacity remaining after the \(promptTokens)-token prompt and required runtime reserve. Reduce the message content."
            parameter = parameterName
            code = "context_length_exceeded"
        }
        ...
        if let originalPrompt {
            headers["X-Original-Prompt-Tokens"] = String(originalPrompt.tokens)
        }
        if let effectivePrompt {
            headers["X-Effective-Prompt-Tokens"] = String(effectivePrompt.tokens)
            headers["X-Token-Count-Mode"] = effectivePrompt.mode.rawValue
        }
        await error(
            connection,
            status: 400,
    ...
    private func listOpenAIModels(_ connection: NWConnection) async {
        ...
        let payload = LocalAPIResponse.json([
            ...
            "data": [[
                ...
                "owned_by": "on-device",
                "root": model.repoID,
                "context_length": profile.effectiveContextWindow,
                "context_window": profile.effectiveContextWindow,
                "max_context_length": profile.effectiveContextWindow,
                "max_input_tokens": profile.effectiveContextWindow,
                "context_window_tokens": profile.effectiveContextWindow,
                "configured_context_length": profile.configuredContextLength,
                ...
                "default_max_tokens": min(profile.defaultOutputTokens, modelSnapshot.4),
                "token_count_mode": profile.runtimeLimits.tokenCountMode.rawValue,
    ...
    private func aiderConfiguration(_ connection: NWConnection) async {
        let snapshot = await MainActor.run {
            (
                CodingAssistantService.shared.activeModel,
                AppSettings.shared.localAPIToolCallingEnabled,
                CodingAssistantService.shared.isVisionChatCapable,
                CodingAssistantService.shared.localAPIEffectiveMaximumOutputTokens
            )
        }
        let profile = ModelCapabilityProfile.resolve(for: snapshot.0)
        let export = AiderConfigurationExporter.export(
            modelID: snapshot.0.id,
            profile: profile,
            supportsFunctionCalling: snapshot.1 && profile.toolParser != .none,
            supportsVision: snapshot.2,
            effectiveMaximumOutputTokens: snapshot.3
        )
        let payload = LocalAPIResponse.json([
            "model": export.modelName,
            "metadata_filename": ".aider.model.metadata.json",
            "metadata_json": export.metadataJSON,
            "settings_filename": ".aider.model.settings.yml",
            "settings_yaml": export.settingsYAML,
            "launch_command": export.launchCommand,
            "context_window": profile.effectiveContextWindow,
            "max_output_tokens": snapshot.3,
            "default_max_tokens": min(profile.defaultOutputTokens, snapshot.3)
        ])
        await respond(connection, status: 200, contentType: "application/json", data: payload)
    }
    ...
    private func openAIModelObject(
        ...
        return [
            ...
            "default_max_tokens": min(profile.defaultOutputTokens, effectiveMaximumOutputTokens),
            "token_count_mode": profile.runtimeLimits.tokenCountMode.rawValue
        ]
    }
    ...
    /// Terminate a stream that already committed to a 200 response with an
    /// in-band error event. HTTP error statuses are only possible before the
    /// streaming headers go out.
    private func sendStreamingErrorEvent(
        _ connection: NWConnection,
        dialect: Dialect,
        type: String,
        message: String
    ...
    private func sendAnthropicToolCompletion(
        ...
        try await send(connection, LocalAPIResponse.anthropicEvent(
            ...
            object: [
                ...
                "usage": [
                    "input_tokens": usage?.promptTokens ?? inputTokens ?? 0,
                    "output_tokens": usage?.completionTokens ?? 0
                ]
            ]
        ))
    ...
    private func hasCommittedStreamingHeaders(_ connection: NWConnection) -> Bool {
        streamingHeadersCommitted.contains(ObjectIdentifier(connection))
    }

    /// Serialized on the actor so the tool-call heartbeat, token path, and
    /// finisher cannot each emit a 200 preamble.
    private func ensureStreamingHeaders(
        _ connection: NWConnection,
        dialect: Dialect,
        model: String,
        ...
        guard !streamingHeadersCommitted.contains(id) else { return }
        streamingHeadersCommitted.insert(id)
    ...
    private func writeStreamingHeaders(
        ...
        header += "Access-Control-Expose-Headers: X-Token-Usage-Approx, X-Token-Count-Mode, X-Model-Context-Window, X-Original-Prompt-Tokens, X-Effective-Prompt-Tokens, X-Requested-Max-Tokens, X-Effective-Max-Tokens, X-Input-Truncated, X-Truncated-Message-Count\r\n"
        header += "Access-Control-Allow-Private-Network: true\r\n"
        header += "Transfer-Encoding: chunked\r\n"
        header += "Connection: close\r\n"
        for (name, value) in headers {
            guard value.unicodeScalars.allSatisfy({ $0.value >= 0x20 && $0.value != 0x7F }) else { continue }
            header += "\(name): \(value)\r\n"
        }
        header += "\r\n"
        try? await send(connection, Data(header.utf8))
        // Everything after the blank line is response body, so every send
        // from here on must be chunk-framed. Register only after the header
        // bytes themselves went out raw.
        chunkedStreamConnections.insert(ObjectIdentifier(connection))
        ...
        if dialect == .openAIResponses, let responseID {
            // Official SDKs assemble the response object from the event
            // sequence, so the stream has to open with the item/part events
            // the deltas attach to — not jump straight to the first delta.
            let inProgressResponse = (try? JSONSerialization.jsonObject(
                with: LocalAPIResponse.openAIResponse(
                    id: responseID,
                    model: model,
                    text: "",
            ...
            try? await send(connection, LocalAPIResponse.openAIResponseEvent(
                "response.created",
                fields: ["response": inProgressResponse]
            ))
            try? await send(connection, LocalAPIResponse.openAIResponseEvent(
    ...
    private func error(
        ...
        if dialect == .anthropic {
            ...
            if let parameter { object["param"] = parameter }
            if let code { object["code"] = code }
            payload = LocalAPIResponse.json(["error": object])
        } else {
    ...
    /// Frame a body fragment as a single HTTP/1.1 chunk.
    ...
    /// Split streamed tool-call arguments into bounded fragments so clients
    /// accumulate them as incremental deltas, the way the hosted APIs emit
    /// them, instead of one unbounded arguments blob.
    static func fragmented(_ text: String, chunkSize: Int = 48) -> [String] {
        guard !text.isEmpty else { return [] }
        let characters = Array(text)
        guard characters.count > chunkSize else { return [text] }
        var fragments: [String] = []
        var start = 0
        while start < characters.count {
            let end = min(characters.count, start + chunkSize)
            fragments.append(String(characters[start..<end]))
            start = end
        }
        return fragments
    }
    ...
    private func sendFinal(_ connection: NWConnection, _ data: Data) async throws {
        let connectionID = ObjectIdentifier(connection)
        var payload = data
        if chunkedStreamConnections.contains(connectionID) {
            payload = Self.chunkFrame(data)
            // The zero-length chunk terminates the chunked body. The FIN from
            // `isComplete` then closes an already-complete response instead of
            // delimiting an open-ended one.
...
struct HTTPRequest {
    ...
    /// True when the body uses `Transfer-Encoding` framing the request parser
    /// cannot decode. Callers answer these with 411 so chunked-uploading
    /// clients learn the server needs a Content-Length instead.
    var declaresChunkedBody: Bool {
        guard let transferEncoding = headers["transfer-encoding"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
              !transferEncoding.isEmpty,
              transferEncoding.lowercased() != "identity" else {
            return false
        }
        return true
    }
