...
struct CoreAIModelManagementView: View {
    ...
    /// Loads are driven from several places (this button, downloads'
    /// auto-load, Overview). Reflecting the shared service state keeps the
    /// button honest instead of silently no-oping on a second tap.
    private var isLoadInFlight: Bool {
        if case .loading = assistant.state { return true }
        return false
    }
    ...
    private var loadStatusText: String {
        if case .loading(let detail) = assistant.state {
            return detail
        }
        return "Preparing model…"
    }

    private func removeInstalledModel() {
        do {
            try store.removeModel()
            CodingAssistantService.shared.unload()
            ToastCenter.shared.success("Model removed")
        } catch {
                ...
                if case .ready(_, let manifest) = store.state {
                    ...
                    HStack(spacing: 8) {
                        Button {
                            ...
                            Label(loadButtonTitle, systemImage: "bolt.fill")
                                .frame(maxWidth: .infinity)
                        }
                        .buttonStyle(LASPrimaryButtonStyle())
                        .disabled(isLoadInFlight)
                        ...
                        .confirmationDialog(
                            ...
                            Text("The model resources are deleted from this device. You can download or import the pack again later.")
                        }
                    }
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            DisclosureGroup(isExpanded: $showExactRepo) {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    .buttonStyle(LASPrimaryButtonStyle())
                    .disabled(!exactRepoInputIsValid)
                }
                .padding(.top, 8)
            } label: {
    ...
    /// Owner/repo ids are typed by hand; reject anything that is not one or
    /// more `owner/name` path components of URL-safe characters before it
    /// can reach the download manager.
    private var exactRepoInputIsValid: Bool {
        let repo = customRepo.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !repo.isEmpty else { return false }
        let allowed = CharacterSet(charactersIn: "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-._/")
        return repo.unicodeScalars.allSatisfy(allowed.contains)
            && repo.split(separator: "/").count >= 2
            && repo.split(separator: "/").allSatisfy { !$0.isEmpty }
    }
    ...
    private var importSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                Button {
                    // Open-in-place folder picking needs a concrete signing
                    // identity; ForgeSign-class wildcard resigns route to the
                    // in-sandbox App Documents flow instead.
                ...
                Text(LocalModelDocumentPickerSession.openInPlacePickingIsUsable
                     ? "Prefer the full resource directory (metadata.json + tokenizer + .aimodel). If Files crashes on sideload builds, copy the pack into Core AI: LAS via the Files app, then use App Documents."
                     : "This sideload build can't open folders from Files directly — copy the pack into Core AI: LAS via the Files app, then use App Documents.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            .frame(maxWidth: .infinity, alignment: .leading)
