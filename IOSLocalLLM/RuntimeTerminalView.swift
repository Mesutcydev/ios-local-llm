...
struct LASTerminalView: View {
    ...
    /// Tab-root embedding (Core AI's Diagnostics tab) has no stack to pop;
    /// suppress the back chevron there.
    var showsBackButton: Bool = true

    var body: some View {
        VStack(spacing: 0) {
            LASDetailHeader("Verbose terminal", dark: true, showsBack: showsBackButton) {
                Button {
                    logCenter.clear()
                } label: {
